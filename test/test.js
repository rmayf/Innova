...
describe( 'Basic Gameplay', function() {
   ...
   describe( 'Turn Flow', function() {
      it( 'Draw Action', function() {
         ...
         try {
            game.drawCard( 11 );
         } catch( victoryCondition ) {
            expect( victoryCondition.players ).to.have.length( 2 );
         }
         player2.scoreCards.push( {age: 1 } );
         try {
            game.drawCard( 11 );
         } catch( victoryCondition ) {
            expect( victoryCondition.players[ 0 ] ).to.equal( player2 );
         }
         player2.scoreCards = [];
         player1.achievements.push();
         try {
            game.drawCard( 11 );
         } catch( victoryCondition ) {
            expect( victoryCondition.players[ 0 ] ).to.equal( player1 );
         }
      } )
   ...
   describe( 'Melding and Splaying', function() {
      it( 'Melding', function() {
         player1.actions = 1;
         expect( player1.hand ).to.have.length( 2 );
